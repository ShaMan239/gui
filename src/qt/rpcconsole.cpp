...
protected:
    virtual void highlightBlock(const QString& text) override
    {
        ...
        QRegularExpressionMatchIterator temp = m_pattern.globalMatch(text);
        int count = 0;
        ...
        // }
        QRegularExpressionMatch match, temp1;
        while (temp.hasNext()) {
            temp.next();
            // std::cout<<count<<"\n";
            count++;
        ...
        if(count != 0)
        {
            ...
            setFormat(match.capturedStart(), match.capturedLength(), m_format);
            outOfString = QString::number(n) + " of " + QString::number(count);
        }
    }
    ...
    platformStyle(_platformStyle)
{
    ...
    connect(ui->nextButton, &QAbstractButton::clicked, this, &RPCConsole::onSearchText);
    connect(ui->searchBar, &QLineEdit::textChanged, this, &RPCConsole::onIndexClean);
...
void RPCConsole::onSearchText()
{
    ...
    ui->outOf->setText(m_searchHighLight->textOf());
}

void RPCConsole::onIndexClean()
{
    m_searchHighLight->indexClean();
    ui->outOf->setText(m_searchHighLight->textOf());
}
